...
fn main() {
    ...
    let mut applicable_games: Vec<usize> = vec![];

    // parse 
    let re = Regex::new(r"^Game\s+(\d+):\s+(?<game>.*)$").unwrap();
    let round_re = Regex::new(r"(?i)(?<round>(?:(\s*(\d+)\s+(green|red|blue),?)+;?))").unwrap();
    let game_re = Regex::new(r"(?i)(?:\s*(\d+)\s+(green|red|blue)[,|;]?)").unwrap();
    
    
    for (i, line) in STR_INPUT.lines().enumerate() {
        let mut should_break_on_game = false;
        
        // parse game line
        let Some(game) = re.captures(line) else {
            continue;
        };

        let rounds = round_re.captures_iter(&game["game"]);

        // print game info
        println!("Game {} ------ {:?}", i, game.get(1));

        if game.get(1).unwrap().as_str().parse::<usize>().unwrap() != i {
            println!("\n\n\n-----------------------------------------------------------------YOOOOOOOO they are not the same!!-----------------------------------------------------------------\n\n\n")
        }

        // start rounds
        for (j, round_capture) in rounds.enumerate() {

            // print round info
            println!("\tRound {}: {}", j + 1, &round_capture["round"]);

            let mut visited_colour: HashMap<&str, i32> = HashMap::new();

            for cube_count in game_re
                .captures_iter(&round_capture["round"]) {

                    // check if the round is good with the limits of the cube
                    // parse number of cube count
                    let n_cube_count = cube_count.get(1).unwrap().as_str().parse::<i32>().unwrap();
                    let cube_color = cube_count.get(2).unwrap().as_str();

                    // parse color
                    match cube_color {
                        "green"  => {
                            if n_cube_count > max_green { 
                                println!("GAME NOT POSSIBLE! Due to green {} - Max {}", n_cube_count, max_green); 
                                should_break_on_game = true; 
                                break; 
                            }
                            visited_colour.insert("green", n_cube_count);
                        },
                        "blue" => {
                            if n_cube_count > max_blue { 
                                println!("GAME NOT POSSIBLE Due to blue {} - Max {}", n_cube_count, max_blue); 
                                should_break_on_game = true; 
                                break; 
                            }
                            visited_colour.insert("blue", n_cube_count);
                        },
                        "red" => {
                            if n_cube_count > max_red { 
                                println!("GAME NOT POSSIBLE! Due to red {} - Max {}", n_cube_count, max_red); 
                                should_break_on_game = true; 
                                break; 
                            }
                            visited_colour.insert("red", n_cube_count);
                        }
                        _ => {}
                    }
            }

            if should_break_on_game {
                break;
            }
        }

        if !should_break_on_game {
            applicable_games.push(i);
        }
        ...
        println!("Games applicable: {:?}", applicable_games);
        

        let mut total_games = 0;
        for game_num in applicable_games.iter() {
            total_games += game_num;
        }
        println!("Total: {}", total_games);
}


const STR_INPUT: &str = r#"
