...
fn main() {
    let mut game_power_total:Vec<i64> = vec![];
    ...
    for (i, line) in STR_INPUT.lines().enumerate() {
            ...
            for cube_count in game_re
                .captures_iter(&round_capture["round"]) {
                    ...
                    match cube_color {
                        "green"  => { 
                            if min_green == 0 { 
                                min_green = n_cube_count; 
                                continue;
                            }
                            
                            if n_cube_count > min_green {
                                min_green = n_cube_count;
                            }
                        },
                        "blue" => { 
                            if min_blue == 0 { 
                                min_blue = n_cube_count; 
                                continue;
                            }
                            
                            if n_cube_count > min_blue {
                                min_blue = n_cube_count;
                            }
                        },
                        "red" => { 
                            if min_red == 0 { 
                                min_red = n_cube_count; 
                                continue;
                            }
                            
                            if n_cube_count > min_red {
                                min_red = n_cube_count;
                            }
                        },
                        _ => {}
        ...
        // now we have all min values, they will be multiplied together
        let total_power = min_blue * min_red * min_green;
        println!("---------------- Game min values blue: {} red: {} green: {} ---> power: {}", min_blue, min_red, min_green, total_power);
        game_power_total.push(total_power.into());
    ...
    println!("Games total power: {:?}", game_power_total);
    
    let mut total_sum = 0;
    for i in game_power_total.iter() {
        total_sum += i;
    }
