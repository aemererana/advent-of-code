...
fn main() {
    ...
    let number_str_regex_pattern = "one|two|three|four|five|six|seven|eight|nine";

    let start_digit_re = Regex::new(format!(r#"(?i)(\d|{}).*$"#, number_str_regex_pattern).as_str()).unwrap();
    let end_digit_re = Regex::new(format!(r#"(?si)^.*(\d|{})"#, number_str_regex_pattern).as_str()).unwrap();

    // split text in lines
    for (i, line) in STRING_INPUT.lines().enumerate() {
        println!("Line {} {:?}", i, line);

        let Some(start_digit) = start_digit_re.captures(line) else {
            println!("");
            continue;
        };
        let Some(end_digit) = end_digit_re.captures(line) else {
            println!("");
            continue;
        };

        let parse_num = |number_str: &str| -> i32 {
            match number_str.parse::<i32>() {
                Ok(beginning_number) => beginning_number,
                Err(_) => {
                    match number_str.to_lowercase().as_str() {
                        "one" => 1,
                        "two" => 2,
                        "three" => 3,
                        "four" => 4,
        ...
        // parse int from str
        let mut first_num = 0;
        let start_digit_str = start_digit.get(1).unwrap().as_str();
        first_num = parse_num(start_digit_str);

        let mut second_num = 0;
        let second_digit_str = end_digit.get(1).unwrap().as_str();
        second_num = parse_num(second_digit_str);
        ...
        println!("Digits {}\n    {:?} {:?}", digit, first_num, second_num);

        sum_total += digit;


        // separate a new line
        println!("Current sum: {}", sum_total);
        println!("");
    }
}

const STRING_INPUT: &str = r#"
