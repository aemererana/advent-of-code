...
fn main() {
    ...
    let end_digit_re = Regex::new(r"(?s)^.*(\d)").unwrap();
    let start_digit_re = Regex::new(r"(\d).*$").unwrap();
    ...
    for (i, line) in STRING_INPUT.lines().enumerate() {
        ...
        println!("Digits {:?} {:?}", start_digit, end_digit);
        let digit = format!(
            "{}{}", 
            start_digit.get(1).unwrap().as_str(), 
            end_digit.get(1).unwrap().as_str())

        .parse::<i32>().unwrap();

        println!("Digits {}\n    {:?} {:?}", digit, start_digit, end_digit);
